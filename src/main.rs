...
impl Parser {
    ...
    fn parse_breach(&mut self) -> Value {
        ...
        let interface = interfaces.into_iter()
            ...
            .expect("❌ Host-Only interface lost.");

        let source_mac = interface.mac.unwrap();
        let dest_mac = MacAddr(0x08, 0x00, 0x27, 0x62, 0x85, 0x77); 
        let source_ip = Ipv4Addr::new(192, 168, 56, 1);
        let dest_ip: Ipv4Addr = target_ip_str.parse().expect("Invalid IP format");
        ...
        let syn_packet = forge_packet(source_ip, dest_ip, source_mac, dest_mac, 1000, 0, TcpFlags::SYN, &[]);
        tx.send_to(&syn_packet, None).unwrap().unwrap();
                    ...
                    if let Some(eth) = EthernetPacket::new(packet) {
                        if let Some(ipv4) = Ipv4Packet::new(eth.payload()) {
                            if let Some(tcp) = TcpPacket::new(ipv4.payload()) {
                                if tcp.get_source() == 80 && tcp.get_flags() == (TcpFlags::SYN | TcpFlags::ACK) {
        ...
        Value::Gateway { 
            ...
            next_seq: hijacked_ack, 
            next_ack: hijacked_seq + 1 
        }
    }
    ...
    fn parse_pipe_injection(&mut self, gateway_id: String) {
        ...
        let payload_str = if let Value::Str(s) = payload_val { s } else { panic!("Payload must be a string"); };
    ...
    fn parse_assignment(&mut self) {
        ...
        let name = if let Some(Token::Identifier(id)) = self.peek() { self.next(); id } else { panic!("Invalid assignment"); };
        ...
        let val = if let Some(Token::Keyword(k)) = self.peek() {
            match k.as_str() {
                ...
                "input" => self.parse_input(), "breach" => self.parse_breach(),
                _ => self.parse_cond(),
            }
